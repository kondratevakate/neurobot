...
def print_results_(clf_grid_dict, save_plot_to=None, problem_name=None):
    ...
    cnt=0
    fig, axs = plt.subplots(len(results["classifier"]), figsize=(8,14))
    for clf in results["classifier"]:
        ...
        cnt+=1
    if save_plot_to is not None:
        fig.savefig('{}{}_{}_folds.png'.format(save_plot_to,
                                                today,
                                                problem_name
                                               ), bbox_inches='tight')
    plt.show()
...
class GridCVLinear:
    ...
    def print_results_(self, save_plot_to=None):
        clf_grid_dict = dict(zip(self.classifiers, 
                self.grid))
    ...
    def bootstrap_632(self, n_splits = 100):
